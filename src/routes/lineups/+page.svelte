<script>
    ...
    const tableColumns = [
        { key: 'lineup_label', label: 'Lineup', alignClass: 'lineup-col', dataType: 'text' },
        { key: 'team_name', label: 'Team', alignClass: 'team-col', dataType: 'text' },
        { key: 'possessions', label: 'Poss', alignClass: 'num', dataType: 'number' },
        { key: 'net_pm', label: 'Net +/-', alignClass: 'num', dataType: 'number' },
        { key: 'off_pm', label: 'Off +/-', alignClass: 'num', dataType: 'number' },
        { key: 'def_pm', label: 'Def +/-', alignClass: 'num', dataType: 'number' }
    ];
    ...
    function formatCellValue(row, column) {
        ...
        if (column.key === 'net_pm' || column.key === 'off_pm' || column.key === 'def_pm') {
...
<div class="container">
    ...
    {:else}
        <p class="variant-summary">Showing {selectedVariantLabel} lineup ratings.</p>
                ...
                <tbody>
                    {#each sortedLineups as lineup (lineup.row_key)}
                        <tr>
                            {#each tableColumns as column (column.key)}
                                <td
                                    class="{column.alignClass} {column.key === 'net_pm' || column.key === 'off_pm' || column.key === 'def_pm' ? metricToneClass(lineup[column.key]) : ''}"
                                >
                                    {#if column.key === 'lineup_label'}
                                        <span class="lineup-label">{lineup.lineup_label}</span>
                                    {:else if column.key === 'team_name'}
                                        <span class:team-placeholder={lineup.team_name === TEAM_PENDING_LABEL}>
                                            {lineup.team_name}
                                        </span>
                                    {:else}
